...
export const ValidationErrorMessages = {
  ...
  login: {
    ...
    invalidFormat:
      'Login must start with a letter and contain only English letters',
  },
  password: {
...
export function validateLogin(login: string): string | null {
  ...
  if (!loginRegex.test(login))
    return ValidationErrorMessages.login.invalidFormat;
  return null;
}
...
export function validateUserName(name: string): string | null {
  ...
  if (!onlyEnRegex.test(name))
    return ValidationErrorMessages.username.invalidChars;
  return null;
}
...
export function validatePassword(password: string): string | null {
  ...
  if (!specialCharRegex.test(password))
    return ValidationErrorMessages.password.missingSpecial;
  return null;
}
...
export function validateConfirmPassword(
  ...
  if (password !== confirmPassword)
    return ValidationErrorMessages.confirmPassword.notMatch;
  return null;
}
